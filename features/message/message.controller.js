...
const socketController = (socket) => {
    ...
    try {
      ...
      if (!fromUserExists) {
        socket.emit("sendMessageError", {
          error: `User with ID ${fromUserId} does not exist.`,
        });
        return;
      }
      const toUserExists = await userService.getUserById(toUserId);
      if (!toUserExists) {
        socket.emit("sendMessageError", {
          error: `User with ID ${toUserId} does not exist.`,
        });
        return;
      }
      console.log("Successful", fromUserId, toUserId, message);
  ...
  socket.on("countMessage", async (data) => {
    ...
    try {
      ...
      const messages = await MessageService.getMessagesByUserIdCount(
        touserId,
        fromuserId
      );
      const unreadMessages = messages.filter(
        (message) => message.notRead === true
      );
