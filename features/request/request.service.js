...
import { friendRequestStatus, accountType } from "../../config/enum.js";
...
class FriendRequestService {
  ...
  async sendFriendRequest(senderId, receiverId) {
    ...
    const existingRequestSent = await FriendRequest.findOne({
      from: senderId,
      to: receiverId,
    });
    ...
    const existingRequestReceived = await FriendRequest.findOne({
      from: receiverId,
      to: senderId,
    });
      ...
      if (newStatus === friendRequestStatus.ACCEPTED) {
        ...
        if (newStatus === "Accepted") {
          notificationResponse =
            await notificationService.acceptFriendRequestNotification(
              senderId,
              receiverId
            );
        }
        ...
        return {
          success: true,
          message: "Friend request accepted and successfully",
          notificationResponse,
        };
