...
class FriendRequestController {
  ...
  static async UpdateFriendRequestStatus(req, res) {
    ...
    try {
      ...
      const updatedRequest =
        await friendRequestService.updateFriendRequestStatus(
          friendRequestId,
          status
        );
      return res
    ...
    try {
      ...
      if (!user) {
        ...
        return res
          ...
          .json({
            success: true,
            message: "following Retived Succesfully",
            data: user.following,
          });
      }
    } catch (error) {
  ...
  static async removeFollowers(req, res) {
    ...
    const senderId = req.user._id;
    try {
      const result = await friendRequestService.removeFollowers(
        senderId,
        userId
      );
      if (!result.success) {
  ...
  static async removeFollowings(req, res) {
    ...
    const senderId = req.user._id;
    try {
      const result = await friendRequestService.removeFollowings(
        senderId,
        userId
      );
      if (!result.success) {
