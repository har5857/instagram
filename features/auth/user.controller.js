...
import fs from "fs";
// import path from 'path';
import { accountType } from "../../config/enum.js";
...
class UserController {
  ...
  static async registerUser(req, res) {
    try {
      ...
      if (req.files["profilePic"] && req.files["profilePic"].length > 1) {
        return res
          .status(400)
          .json({ message: "Only 1 file allowed for profilePic." });
      }
      if (
        req.files["profilePics"] &&
        (req.files["profilePics"].length < 2 ||
          req.files["profilePics"].length > 5)
      ) {
        return res
          .status(400)
          .json({
            message: "Minimum 2 and maximum 5 files allowed for profilePics.",
          });
      }
      let hashPassword = await bcrypt.hash(req.body.password, 10);
      let role = "User";
      ...
      return res
        .status(200)
        .json({ success: true, message: `User Register Succesfully`, user });
    } catch (error) {
      console.error("Error registering user:", error);
      res
        .status(500)
  ...
  static async resendOtp(req, res) {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, message: "New OTP Sent Successfully", otp });
    } catch (error) {
  ...
  static async getUser(req, res) {
    try {
      ...
      if (!user.isDelete) {
        if (
          user.followers != requestingUserId &&
  ...
  static async activeDeactiveUser(req, res) {
    try {
      ...
      let updatedIsDelete;
      if (isDelete.toLowerCase() === "false") {
        updatedIsDelete = false;
      } else if (isDelete.toLowerCase() === "true") {
        updatedIsDelete = true;
      } else {
        return res
          .status(400)
          .json({ message: 'Invalid isDelete value. Use "false" or "true".' });
      }
      user = await userService.updateUser(userId, {
        isDelete: updatedIsDelete,
      });
      res.status(200).json({
        success: true,
        message: updatedIsDelete
  ...
  static async removeProfilePics(req, res) {
    try {
      ...
      const profilePicsIndex = user.profilePics.findIndex(
      ...
      if (!picturePath) {
        return res
          .status(404)
          .json({ message: "Profile picture path not found" });
      }
      const relativePath = picturePath.replace(
        /^http:\/\/localhost:5555\/uploads\/profile_picture\//,
        ""
      );
      const filePath = path.join(
        "D:/instagram/uploads/profile_picture",
        relativePath
      );
      ...
      res
        ...
        .json({
          success: true,
          message: "ProfilePics Deleted Successfully",
          data: userObj,
        });
    } catch (error) {
      console.error(error);
  ...
  static async forgotPassword(req, res) {
    ...
    try {
      ...
      const resetLink = `http://192.168.29.229:5555/api/user/reset-password?token=${resetToken}`;
